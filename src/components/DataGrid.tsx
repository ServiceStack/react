    ...
    const slots = useMemo(() => {
        ...
        React.Children.forEach(children, child => {
            if (React.isValidElement(child) && child.props.slot) {
                slotMap[child.props.slot] = child
            }
        })
        return slotMap
    }, [children])
                                ...
                                <tr className={theadRowClass}>
                                    {visibleColumns.map(column => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div onClick={(e) => handleHeaderSelected(e, column)}>
                                                    {slots[column + '-header'] ?
                                                        slots[column + '-header'] :
                                                        headerSlotName && slots[headerSlotName] ?
                                                            slots[headerSlotName] :
                                                            slots.header ?
                                                                React.cloneElement(slots.header as React.ReactElement, { column, label: headerFormat(column) }) :
